                    ...
                    <form action="{{ route('berita.update', $berita) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div>
                            <label for="information_description"
                                class="block text-gray-700 text-sm font-medium">Deskripsi Berita</label>
                            <textarea name="information_description" id="information_description" rows="6"
                                class="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                required>{{ old('information_description', $berita->information_description) }}</textarea>
                        ...
                        @if ($berita->information_head_cover)
                            <div class="mt-4">
                                ...
                                <img src="{{ asset('img/' . $berita->information_head_cover) }}" alt="Current cover"
                                    class="w-40 mt-2 rounded-md border border-gray-300 shadow">
                            </div>
                        @endif

                        <div class="mt-6 flex justify-between gap-4">
                            <button type="button" onclick="window.location='{{ route('berita') }}'"
