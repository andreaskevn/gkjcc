        ...
        <div class="flex-1 flex flex-col md:ml-64">
            ...
            <main class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                <div class="w-full max-w-4xl bg-white p-10 rounded-2xl shadow-xl">
                    ...
                    <form action="{{ route('jadwalibadah.store') }}" method="POST" enctype="multipart/form-data"
                        class="space-y-6">
                        @csrf
                            ...
                            <div class="w-1/2">
                                ...
                                <select name="worship_schedule_day" id="worship_schedule_day" required
                                    ...
                                    @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
                                        <option value="{{ $hari }}"
                                            {{ old('worship_schedule_day') == $hari ? 'selected' : '' }}>
                                            {{ $hari }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="worship_schedule_language" id="worship_schedule_language" required
                                ...
                                <option value="Bahasa Indonesia"
                                    {{ old('worship_schedule_language') == 'Bahasa Indonesia' ? 'selected' : '' }}>
                                    Bahasa Indonesia
                                </option>
                                <option value="Bahasa Jawa"
                                    {{ old('worship_schedule_language') == 'Bahasa Jawa' ? 'selected' : '' }}>
                                    Bahasa Jawa
                                </option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pastor_id" id="pastor_id" required
                                ...
                                @foreach ($pastor as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('pastor_id') == $item->id ? 'selected' : '' }}>
                                        {{ $item->pastor_name }}
                                    </option>
                                @endforeach
                            </select>
