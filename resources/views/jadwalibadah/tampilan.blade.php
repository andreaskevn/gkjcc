...
<head>
    ...
    <title>GKJCC | Jadwal Ibadah</title>
    ...
    <div class="flex min-h-screen relative">
        ...
        <div class="flex-1 flex flex-col md:ml-64">
            ...
            <header
                ...
                <h1 class="text-lg font-semibold">GKJCC | Jadwal Ibadah</h1>
                ...
                <div class="bg-white shadow-xl rounded-2xl overflow-hidden mb-6">
                    ...
                    <div class="bg-gradient-to-r from-blue-600 to-purple-600 p-6">
                        <div class="flex justify-between items-center">
                            <h1 class="text-2xl font-bold text-white">Daftar Jadwal Ibadah</h1>
                            <a href="{{ route('jadwalibadah.create') }}"
                    ...
                    <div class="p-6">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                            <div class="flex space-x-4 w-full">
                                ...
                                <div class="w-full md:w-auto">
                                    <x-limit route="jadwalibadah" />
                                </div>
                                <div class="w-full sm:w-auto">
                                    <x-search route="jadwalibadah" />
                                </div>

                                {{-- Filter Dropdown --}}
                                <form action="{{ route('jadwalibadah') }}" method="GET" id="filterForm"
                                    ...
                                    <select name="category_id"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                ...
                                                {{ $category->worship_schedule_category_name }}
                                            </option>
                                        @endforeach
                                    </select>

                                    <select name="pastor_id" onchange="document.getElementById('filterForm').submit();"
                                        class="border px-3 py-2 rounded-md">
                                        <option value="">- Semua Pendeta -</option>
                                        @foreach ($pastors as $pastor)
                                            <option value="{{ $pastor->id }}"
                                                {{ $pastorFilter == $pastor->id ? 'selected' : '' }}>
                                                {{ $pastor->pastor_name }}
                                            </option>
                                        @endforeach
                                    </select>

                                    {{-- Tambahkan hidden input agar limit & search tetap terbawa --}}
                            ...
                            <table class="w-full hidden md:table">
                                <thead>
                                    <tr class="bg-gray-100 text-left text-sm font-semibold uppercase tracking-wide">
                                        <th class="py-3 px-6 text-left">Nama Jadwal Ibadah</th>
                                        <th class="py-3 px-6 text-left">Hari</th>
                                        <th class="py-3 px-6 text-left">Jam</th>
                                        <th class="py-3 px-6 text-left">Bahasa</th>
                                        <th class="py-3 px-6 text-left">Kategori</th>
                                        <th class="py-3 px-6 text-left">Pendeta</th>
                                ...
                                <tbody>
                                    @foreach ($jadwalibadah as $item)
                                        <tr class="border-b border-gray-300 hover:bg-gray-50 transition duration-200">
                                            <td class="py-3 px-6">{{ $item->worship_schedule_name }}</td>
                                            <td class="py-3 px-6">{{ $item->worship_schedule_day }}</td>
                                            <td class="py-3 px-6">{{ $item->worship_schedule_hour->format('H:i') }}
                                                WIB</td>
                                            <td class="py-3 px-6">{{ $item->worship_schedule_language }}</td>
                                            <td class="py-3 px-6">
                                                {{ $item->category->worship_schedule_category_name }}</td>
                                            <td class="py-3 px-6">Pdt. {{ $item->pastor->pastor_name }}</td>
                                            <td class="py-3 px-6">
                                                <a href="{{ route('jadwalibadah.edit', $item->id) }}"
                                                    class="bg-yellow-500 text-white px-4 py-1 rounded hover:bg-yellow-600">Edit</a>
                                                <form action="{{ route('jadwalibadah.destroy', $item->id) }}"
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        data-id="{{ $item->id }}"
                                                        data-name="{{ $item->worship_schedule_name }}">Hapus</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="md:hidden">
                            ...
                            <div class="bg-white rounded-lg shadow-md mb-4 p-4">
                                <div class="text-lg font-semibold text-gray-800 mb-2">
                                    {{ $item->worship_schedule_name }}
                                </div>
                                <div class="grid grid-cols-1 gap-4 text-left">
                                    <p><span class="font-semibold">Hari:</span> {{ $item->worship_schedule_day }}</p>
                                    <p><span class="font-semibold">Jam:</span> {{ $item->worship_schedule_hour }}</p>
                                    <p><span class="font-semibold">Bahasa:</span> {{ $item->worship_schedule_language }}</p>
                                    <p><span class="font-semibold">Kategori:</span> {{ $item->category->Kategori }}</p>
                                    <p><span class="font-semibold">Pendeta:</span> {{ $item->pastor->Pendeta }}</p>
                                </div>
                                <div class="flex gap-2 mt-4">
                                    <a href="{{ route('jadwalibadah.edit', $item) }}"
                                        class="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 transition duration-300 flex-1 text-center">
                                        Edit
                                    </a>
                                    <form action="{{ route('jadwalibadah.destroy', $item->id) }}" method="POST" class="inline delete-form flex-1">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition duration-300 w-full delete-button"
                                            data-id="{{ $item->id }}" data-name="{{ $item->worship_schedule_name }}">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                        ...
                        <div class="mt-6 flex justify-center">
                            {{ $jadwalibadah->appends(['search' => request('search'), 'limit' => request('limit')])->links('pagination::tailwind') }}
                        </div>
                    </div>
                </div>
            </main>
