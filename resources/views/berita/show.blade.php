...
<head>
    ...
    <title>{{ $berita->information_title }}</title>
    ...
    <div class="min-h-screen flex justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl w-full bg-white p-8 rounded-lg shadow-lg">
            ...
            <p class="text-sm text-gray-600 mb-6">
                {{ $berita->user->name ?? 'Unknown' }} – {{ $berita->created_at->locale('id')->translatedFormat('l, d F Y') }}
            </p>

            <!-- Gambar -->
            @if ($berita->information_head_cover)
                <img src="{{ asset('img/' . $berita->information_head_cover) }}" alt="Cover Berita"
