...
@section('title', $berita->information_title)
@section('content')
    <div class="bg-white py-8 px-4 sm:px-8 max-w-4xl mx-auto shadow-md rounded-lg mt-6">
        ...
        @if ($berita->information_head_cover)
            <div class="my-6 flex justify-center">
                <img src="{{ asset('img/' . $berita->information_head_cover) }}" alt="Cover"
                    class="rounded-lg shadow-lg w-full max-w-3xl object-cover">
            </div>
        @endif

        <div class="text-gray-800 leading-relaxed text-justify space-y-4">
            {!! nl2br(e($berita->information_description)) !!}
        </div>

        @if ($rekomendasi->count())
            <div class="max-w-6xl mx-auto mt-16">
                <h2 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Baca Juga Berita Lainnya</h2>
